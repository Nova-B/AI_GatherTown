...
export const DEMO_STEPS: DemoStep[] = [
  ...
  // The lead delegates twice (Agent tool); the children start right after. The
  // prompt is not part of the payload, only description/subagent_type/model.
  { after: 500, provider: 'claude', payload: { session_id: CLAUDE_SID, hook_event_name: 'PreToolUse', cwd: CWD_A, tool_name: 'Agent', tool_use_id: 'toolu_a1', tool_input: { description: '세션 쿠키 처리 코드 탐색', subagent_type: 'Explore' } } },
  { after: 200, provider: 'claude', payload: { session_id: CLAUDE_SID, hook_event_name: 'PreToolUse', cwd: CWD_A, tool_name: 'Agent', tool_use_id: 'toolu_a2', tool_input: { description: '로그인 오류 수정 후 auth 테스트', subagent_type: 'general-purpose', model: 'sonnet' } } },
  { after: 300, provider: 'claude', payload: { session_id: CLAUDE_SID, hook_event_name: 'SubagentStart', cwd: CWD_A, agent_id: 'agent-explore-1', agent_type: 'Explore' } },
  ...
  { after: 300, provider: 'claude', payload: { session_id: CLAUDE_SID, hook_event_name: 'PostToolUse', cwd: CWD_A, tool_name: 'Agent', tool_use_id: 'toolu_a1', tool_input: { description: '세션 쿠키 처리 코드 탐색', subagent_type: 'Explore' }, tool_response: {} } },
  ...
  { after: 300, provider: 'claude', payload: { session_id: CLAUDE_SID, hook_event_name: 'PostToolUse', cwd: CWD_A, tool_name: 'Agent', tool_use_id: 'toolu_a2', tool_input: { description: '로그인 오류 수정 후 auth 테스트', subagent_type: 'general-purpose', model: 'sonnet' }, tool_response: {} } },
