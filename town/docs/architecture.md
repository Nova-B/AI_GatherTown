...
~/.claude/projects/<cwd>/<session>.jsonl ──tail, marker only──▶ server/transcript.ts ──synthesized turn.failed──▶ same store/broadcast path
```
...
| Esc (no end event) | Claude Code fires no Stop when the user interrupts, and no idle_prompt follows. Primary detection: the server's transcript watcher (`server/transcript.ts`) sees the `[Request interrupted by user…]` user line Claude appends to the session transcript and synthesizes `turn.failed` (`interrupted`, source `transcript`, evidence `observed`) for the running turn, within about a second of the poll. Fallback (inferred, `endEvidence: 'inferred'`): a new `turn.started` with a different turn id, or an `idle_prompt`/`agent_needs_input` reaching the root agent with no pending approval. In every case all agents' running tools become `unresolved`, pending approvals resolve as inferred, active agents go idle with `lastResponse: 'interrupted'`; any later tool activity reactivates an agent. A re-delivered start of the same turn id (a message sent mid-turn) is a duplicate, not an interruption |
...
- Transcript watcher: reads only Claude transcripts whose path is derived from a known session's cwd and id (session ids with path separators or `..` are rejected), only bytes appended after the watcher first saw the file, at most 1 MB per second per file, at most 32 recent sessions. It keeps nothing but whether a `type: "user"` line carries the interrupt marker and its timestamp; the synthesized event holds ids, the turn id and a fixed note. Disable with `AGENT_TOWN_TRANSCRIPT_WATCH=0`. Diagnostics show "Esc 감지" with the count of watched sessions and markers found.
