...
- `GET /api/retrospect?session=<key>[&scope=all|last-turn][&from=<seq>][&to=<seq>]` (browser session required) builds the "작업 회고 자료" Markdown from that session's stored events only (`src/shared/retrospect.ts`, pure). No LLM is called and nothing leaves the machine: the user copies the text into the CLI session that did the work. The user's own observations are merged in the browser (`withUserNotes`) and never reach the server. Demo sessions are refused; the response carries only what the store already holds (ids, tool names, masked targets, times, statuses).
...
- Office visibility (`viewModel.isAgentHidden`): an ended session is not drawn at all and its room is freed; an ended agent is not drawn; a finished employee (subagent whose response ended, no pending approval, not waiting for input) stays for `DONE_LINGER_MS` (8 s after its last activity) with a "완료 · {task}" bubble and an observed tool summary ("읽기 3 · 검색 1"), then leaves. The state and the panels keep every session and agent; only the scene hides them. `OfficeView` re-derives the view model once a second so departures happen without a new event; in replay mode the clock is the last replayed event's time, not the wall clock.
...
- Retrospect modal (session details → "작업 회고 자료"): scope (whole session / last turn), a notes box for the user's own observations that becomes §0 of the document, the generated Markdown (read-only), metrics chips and a copy button. In paused/replay views the material stops at the view's last seq. The document lists per-turn timelines and "검토 후보" (serial exploration chains, lead-heavy exploration, light delegations, repeated failures, approval waits, exploration-only delegations on the session model) labelled as pointers, not conclusions, followed by the retrospective questions. Disabled in DEMO.
