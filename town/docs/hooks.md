...
| Notification | yes | approval.requested (`permission_prompt`, no tool id) / notification (`idle_prompt`, `agent_needs_input` → waiting for input) | The id-less prompt record is resolved (inferred) as soon as the agent proceeds or a `PermissionRequest` with an id arrives, and is skipped while an id-ful approval of the agent is pending - otherwise "승인 대기" would outlive the approved command |
| Stop | yes | agent.response_completed (main) → also completes the turn | not session end. **Not fired on Esc** (confirmed in recordings). `idle_prompt` does **not** follow an Esc either (6 min observed without one). The interrupted turn is closed within ~1-3 s by the transcript marker watcher (row below), or by inference on the next UserPromptSubmit |
| (transcript marker, not a hook) | server-side | turn.failed (`interrupted`, source `transcript`, evidence `observed`) | Claude Code appends `{"type":"user", … "[Request interrupted by user…]"}` to `~/.claude/projects/<cwd → '-'>/<session>.jsonl` right after Esc. The server follows that file for sessions with a running turn (1 s poll, appended bytes only, marker only; never prompts or responses). `AGENT_TOWN_TRANSCRIPT_WATCH=0` disables; `AGENT_TOWN_CLAUDE_PROJECTS` overrides the directory. Not a stable format: if the marker changes, detection silently stops and the inference rules still apply |
...
| SubagentStop | yes | agent.response_completed (child) | child shows "완료 · {task}" plus an observed tool summary until the session's next `UserPromptSubmit`, then leaves the office (state and panels keep it); parent turn untouched. `last_assistant_message` is still dropped. Recordings of 2026-09-15 show 8 SubagentStops for 1 SubagentStart: a stop for an id with no start and no tool activity is kept as `startObserved: false` and not drawn |
| PreToolUse `SubagentHandback` | (tool) | tool.started, activity `agent` | Seen in recordings inside a subagent context; labelled "SubagentHandback(결과 넘김)" |
