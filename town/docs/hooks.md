...
| Stop | yes | agent.response_completed (main) → also completes the turn | not session end. **Not fired on Esc** (confirmed in recordings): the interrupted turn is closed by inference on the next UserPromptSubmit or idle_prompt, see architecture "Esc" |
