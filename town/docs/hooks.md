...
| SubagentStop | yes | agent.response_completed (child) | child shows "완료 · {task}" plus an observed tool summary until the session's next `UserPromptSubmit`, then leaves the office (state and panels keep it); parent turn untouched. `last_assistant_message` is still dropped |
